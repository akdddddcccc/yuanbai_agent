...
- 语音排队也由同一个 VPS 服务协调，默认最多同时处理 2 个会话，`YUANBAI_VOICE_CONCURRENCY` 可设为 1–5。其余请求按加入顺序等待，最多保留 200 个等待票据。
- `/api/yuanbai/voice-queue/{join,status,release,claim}` 只接收短期随机票据。前端播放完回答后释放名额；页面关闭或网络中断后，90 秒租约会自动回收。排队在单个服务进程内协调，服务重启会让旧票据失效，页面会提示重新尝试。
